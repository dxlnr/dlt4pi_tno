...
/*
 * This class processes queries by calling the appropriate workflow or service
 */
...
public class queryProcessor {
    ...
    public boolean run(Expr queryExpr, Entity node) {
        ...
        for (String workFlowID : applWorkflows) {
            //Datalog dlPack = DatalogParser.parseDatalogScript(workflows.get(workFlowID).getScriptPath(), repoDirPath);
            //Datalog dlPack = CyberlogicParser.parseScript(workflows.get(workFlowID).getScriptPath(), repoDirPath);
        ...
        serviceInvocation inv = new serviceInvocation(queryExpr);
        inv.process(node, queryExpr.hashCode());        
        if (inv.getResult() == null) {
            System.out.println("\t -> no matching service");
            return false;
        }
        else {
            claim = new claimSpec(queryExpr, inv.getResult(), "*serviceCL*", "*serviceCL*", repoDirPath);
