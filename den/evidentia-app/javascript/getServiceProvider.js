...
async function main() {
    ...
    if(args.length < 1) {
        console.log('Wrong number of arguments - Username is missing')
        process.exit(1);
    }
    if(args.length > 1) {
    ...
    try {
        ...
        const result = await contract.submitTransaction('requestServiceProvider', 'cbmc', '[a,b,c]');

        console.log(`Transaction has been evaluated, result is: ${result.toString()}`);

        const result2 = await contract.submitTransaction('requestServiceProvider', 'infer', '[a,b,c]');
