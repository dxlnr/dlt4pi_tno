...
/**
 * This class handles invocation of a service locally or remotely.  
 * <p> When the service is local, the class directly invokes the service wrapper. 
 * Otherwise, a Client Mode is instantiated to send the query to a remote server that provides the service. 
 * Once the query is processed remotely, its corresponding result is returned.   
 * </p>
 */
public class serviceInvocation {
    ...
    public serviceInvocation(Expr query) {
        ...
        this.query = query;
        //TODO: evidence templates
        this.evidence = "{service:" + this.query.getPredicate() + ", version:1.0, result:okay}";
    }
        
    public void process(Entity node, int claimHashCode) {
        if (node.noDENMode()) {
            this.processNoDEN(node, claimHashCode);
        }
        else {
     ...
     * @param claimHashCode - unique ID of the query/claim
     */
    public void processNoDEN(Entity node, int claimHashCode) {
        if (this.query.getAuthority().getType().equals("unrestricted")) {
            //try local service
            processLocalService(node, claimHashCode);
    ...
    public void processLocalService(Entity node, int claimHashCode) {
        ...
        if (servicePack.containsService(serviceID)) {
            System.out.println("\t -> query processing as a local service");
            System.out.println(servicePack.get(serviceID).toString());
            String serviceInvDirPath = node.getRepoDirPath() + "/claims/claim_" + claimHashCode + "/" + serviceID;
            this.invoke(query, servicePack.get(serviceID).getSignature(), serviceInvDirPath);
            local = true;
        }
        else {
